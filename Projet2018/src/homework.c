...
double femGrainsContactIterate(femGrains *myGrains, double dt, int iter)
{
    ...
    double error = 0.0;
    double gap,rr, rx, ry, nx, ny, vn, dv, dvIn, dvOut, l;
...
void femGrainsUpdate(femGrains *myGrains, double dt, double tol, double iterMax)
{
    ...
    double omega       = 0.3*M_PI; //--> ~1 tour par 10*dt
    double l;
...
//
    ...
    for(i = 0; i < n; i++) {
        ...
        vxfluid = -l*thetaDot*sin(theta);
        vyfluid =  l*thetaDot*cos(theta);

        double fx = m[i] * gx - gamma * vx[i];
        double fy = m[i] * gy - gamma * vy[i];
        vx[i] += vxfluid + fx * dt / m[i];
        vy[i] += vyfluid + fy * dt / m[i];  }
